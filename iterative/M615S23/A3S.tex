...
FIXME COPIED OVER FROM S17 SEMESTER (A5S.tex); ALL IS SUSPECT

\probpts{P12}{5 pts per part}  \ppart{a} Let $A_1 \in \CC^{3\times 3}$ and $A_2 \in \CC^{4\times 4}$ be the matrices in linear systems LS1 and LS2.  The condition numbers are $\cond(A_1)=  2.1$ and $\cond(A_2) = 11.9$.
...
\probpts{P13}{5 pts per part}  \ppart{a}  I wrote these two codes; the online versions have help comments, but here they are stripped for brevity:
