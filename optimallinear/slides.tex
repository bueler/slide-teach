...
\bigskip
\item this algorithm does $O(n)$ flops, on data of size $O(n)$, in any interpretation, so it is optimal
\end{itemize}
\end{frame}
...
\item partial pivoting should be done for numerical stability
   \begin{itemize}
   \item[$\circ$] result: $PA=LU$ where $P$ is a permutation-of-rows operation (matrix)
...
\begin{itemize}
\item \alert{the rest of the talk assumes $A\in\RR^{m\times m}$ is square and invertible}
...
\item however, this ``fast dense solver'' game is not practical
   ...
   \item<2>[$\circ$] also, for the applications I care about, $O(m^{\,2.376})$ is catastrophically slow
   \end{itemize}
\end{itemize}
\end{frame}
