...
\item a \emph{conservation law} is a first-order system for $\bq(t,x) \in \RR^d$ with a given \emph{flux function} $\bbf:\RR^d\to \RR^d$:
  $$\bq_t + \bbf(\bq)_x=0$$
    ...
    \item[$\circ$] if $\bbf$ depends on $\grad \bq$ then \dots not hyperbolic
    \end{itemize}
...
\begin{itemize}
\setlength{\itemindent}{11mm}
\item[example:] \emph{shallow water equations} with
        $$x$$
\item[example:] \emph{isentropic gas equations} with
        $$x$$
\end{itemize}
\end{frame}
