...
\item how does finite volumes (FV) work in 2D?
    \begin{itemize}
    \item[$\circ$] this might clarify how people think and talk about it
    \end{itemize}
\item the ``finite volumes'' are the ``cells''
    \begin{itemize}
    ...
    \end{itemize}
\item there is one equation per cell \dots see next slide

\bigskip
\item for remaining slides, the problem is a scalar conservation law equation for $q(t,x,y)$:
    $$q_t + \Div \bF(q) = 0$$
...
\begin{itemize}
\item the FV method always starts with the same manipulations
\item suppose $V \subset \RR^2$ is a region (cell) in the $x,y$ plane, with volume (area) $|V|$
...
\begin{itemize}
\item the PDE $q_t + \Div \bF(q) = 0$ has become an ODE for $Q_V$:
    $$\frac{dQ_V}{dt} + \frac{1}{|V|} \int_{\partial V} \bF\cdot \bn\,ds = 0$$
\item suppose the computational domain is a rectangle: $\Omega = [a,b] \times [c,d]$
\item my \emph{structured} FV method cuts $\Omega$ into a grid of cells $V=V_{ij}$ (shown)
    \begin{itemize}
...
\begin{itemize}
\item consider advection with $\bF(q) = \left<uq,vq\right>$:
\begin{align*}
\frac{dQ_{ij}}{dt} + \frac{1}{h_x h_y} \int_{\partial V_{ij}} \left<uq,vq\right> \cdot \bn \,ds &= 0
\end{align*}
\item the boundary $\partial V_{ij}$ has 4 sides (faces) $N,E,S,W$; need 4 integrals:
    $$\int_{\partial V_{ij}} \left<uq,vq\right> \cdot \bn \,ds = \int_N + \int_E + \int_S + \int_W \hspace{40mm}$$

    \begin{itemize}
    \item[$\circ$] actually, only compute $N,E$ faces for each

    cell because $S$ is $N$ of cell below, and etc.
    \end{itemize}

\item simplest integral (quadrature) choice uses

the value at the midpoint of the face
\item the simplest (Godunov) ``donor cell''

upwinding method decides which side

of a face contributes the solution value

when computing the flux
\end{itemize}

\vspace{-50mm}
\hfill \includegraphics[width=0.35\textwidth]{figs/bueler11p8upwind}
\end{frame}


\begin{frame}{upwinding in 2D, cont.}
...
\int_N \left<uq,vq\right> \cdot \bn \,ds &= \int_N \left<uq,vq\right> \cdot \left<0,1\right> \,ds \\
   &= \int_{x_i-\frac{h_x}{2}}^{x_i-\frac{h_x}{2}} v(x,y_j+\tfrac{h_y}{2}) q(t,x,y_j+\tfrac{h_y}{2}) \,dx \hspace{40mm} \\
   &\approx h_x\, v(N) \tilde Q  \quad [\text{midpoint rule}]
\end{align*}
\item where, for upwinding,
    $$\tilde Q = \begin{cases} Q_{i,j}   & v(N) \ge 0 \\
                               Q_{i,j+1} & v(N) < 0 \end{cases} \hspace{40mm}$$
\item decisions like this are made at all 4 faces
    \begin{itemize}
    \item[$\circ$] actually, at only two faces per cell
    \end{itemize}
