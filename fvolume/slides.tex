...
\item Riemann solver view easier (perhaps) for
    \begin{itemize}
    \item[$\circ$] generalization to nonlinear systems
    \item[$\circ$] working with the MOL equations (w/o choosing time-stepping)
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item can we do better than Lax-Wendroff for the simple problem $q_t + a q_x=0$?
\item can we have higher accuracy without oscillations?
    \begin{itemize}
    \item[$\circ$] upwinding is only first-order accurate, with $O(\Delta t + \Delta x)$ local truncation error as $\Delta t\to 0$ and $\Delta x \to 0$, but it avoids oscillations
...
\bigskip
\begin{theorem}[\emph{Godunov, 1959}]  A monotonicity-preserving \alert<2>{linear} scheme for the 1D constant-coefficient equation $q_t + a q_x=0$ cannot have second-order (or higher) local truncation error in $x$.\end{theorem}

\item this is the beginning of modern hyperbolic PDE solvers
    \begin{itemize}
    \item[$\circ$] upwinding, Lax-Friedrichs, Lax-Wendroff, leapfrog are all older
    \item[$\circ$] ``high-resolution'' schemes of the 1970s--1990s overcame the barrier
    \item[$\circ$] \alert{how?}
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item since 1990s, a preferred finite volume approach for hyperbolic systems:
\begin{center}
\alert{high-resolution Godunov method}
\end{center}
    \begin{itemize}
    \item[$\circ$] linear systems: acoustics, elasticity, Maxwell's equations
    \item[$\circ$] nonlinear systems: shallow water, compressible gas equations
    \end{itemize}
\item this approach consists of three things:
    \begin{itemize}
    ...
    \item[$\circ$] \alert{Riemann solvers}:
        \begin{itemize}
        \item ``Riemann problem'' has different cell values on each side of a face
        \item a Riemann solver determines local wave structure (rarefaction, shock)
            \begin{description}[abc]
            \item[$\diamond$] provided by the user
            \item[$\diamond$] computes the flux on the face
            \end{description}
        \item can be exact or approximate (e.g.~Roe approximate solver for shallow water)
        \end{itemize}
    \item[$\circ$] \alert{slope limiters}:
        \begin{itemize}
        \item first-order upwinding: solution is constant in a cell (``classical Godunov'')
        \item higher-order: solution is ``reconstructed'' with slope in each cell
        \item the slope is (nonlinearly) limited so translating does not propagate oscillations
        \end{itemize}
    \end{itemize}
\end{itemize}
\end{frame}
