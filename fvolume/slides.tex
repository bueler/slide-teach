...
\begin{center}
\includegraphics[height=35mm]{figs/covers/leveque} \qquad \includegraphics[height=30mm]{figs/covers/hv} \qquad \includegraphics[height=30mm]{figs/covers/bueler}
\end{center}
\end{frame}
...
\item in these slides they have two roles:
  \begin{enumerate}
  \item for \emph{verifying} simulations
      ...
      \end{itemize}
  \item a ``Riemann solver'' for a hyperbolic system uses an exact solution
      \begin{itemize}
      \item[$\circ$] for a discontinuous initial condition
      \item[$\circ$] explaining this is a major purpose of my talk
      \end{itemize}
  \end{enumerate}
\end{itemize}
\end{frame}
...
\item all examples in these slides use fast, but less-elegant, C code
    ...
    \end{itemize}
\item \dots using the Portable Extensible Toolkit for Scientific computing:
...
\item speed is more critical for 2D and 3D space
    \begin{itemize}
    \item[$\circ$] and parallelizability
    \end{itemize}
\end{itemize}
\end{frame}
...
\item \alert{PLEASE} ask lots of questions, about any topic at all
    ...
    \end{itemize}
\item feel free to email after the talk, or regarding future projects
    \begin{itemize}
...
\bigskip
\item upwind and Lax-Wendroff methods were clear to everyone by $\sim$1960
...
\hfill \includegraphics[width=0.4\textwidth]{figs/fvsketch}

\begin{itemize}
\item let $\ds Q_i(t) \approx \frac{1}{\Delta x} \int_\ximhalf^\xiphalf q(t,x)\,dx$
    \begin{itemize}
    \item[$\circ$] versus in a finite difference scheme: $Q_i^n \approx q(t_n,x_i)$
    \end{itemize}
\item let $\Fiphalf(t) \approx f\left(q(t,\xiphalf)\right)$ is flux at \emph{cell face}
\item these interpretations are important!
\end{itemize}
\end{frame}


\begin{frame}{the FV idea 3}
...
\begin{itemize}
\item Q. one detail \dots suppose we want to use limited slopes and Riemann solvers \emph{in the method-of-lines (MOL) framework} \dots how to do this?
\item A. apply the slope calculation and slope-limiter as usual and then compute the flux $F_{i-1/2}$ in terms of this picture:
...
\item for hyperbolic conservation-law systems $\bq_t + \bbf(\bq)_x = 0$
    \begin{itemize}
    \item[$\circ$] linear examples: acoustics, elasticity, Maxwell
...
\item consists of three things:
    ...
    \item[\alert{3.}] \alert{slope (or flux) limiters}
        ...
        \item to achieve higher-order without oscillations:
            ...
            \item[$\diamond$] reverts to first-order at extrema
            \end{description}
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\item for this section, the problem is a scalar conservation law in 2D:
    ...
    $$q_t + (u q)_x + (v q)_y = 0$$
    \end{itemize}
\item we want the solution a region $\Omega\subset \RR^2$, and for some times $0\le t \le T$
\end{itemize}
\end{frame}
...
\item how does the finite volumes (FV) method work in 2D?
    ...
    \end{itemize}
\item the FV method always starts with the same manipulations \dots next slide
\end{itemize}
\end{frame}
...
\item important principle for most 2D (and 3D) FV schemes:

\hfill \emph{the Riemann problem on each face is treated as 1D, normal to the face}

    \begin{itemize}
    \item[$\circ$] thus for advection, first-order upwinding looks only at the component of velocity normal to the face
    \end{itemize}
\item there are two essentially-equivalent modern routes to ``high-resolution'' schemes:
    \begin{itemize}
    \item[$\circ$] \emph{slope limiters} which regard the solution in each cell as not constant, and then modify the slopes to avoid oscillations
    \item[$\circ$] \emph{flux limiters} which compute the flux based on first-order upwinding, with an added term based on a higher-order flux, but limited to avoid oscillations
    \end{itemize}
\end{itemize}
\end{frame}


\begin{frame}{flux-limiter scheme}
...
in the shaded region
    ...
    \end{itemize}
\item similar at $S,W,E$ faces
\item see Sweby (1984)
\end{itemize}

\vspace{-33mm}
\hfill \includegraphics[width=0.6\textwidth]{figs/bueler11p6}
\end{frame}
...
\begin{itemize}
\item compare the decision made with first-order upwinding (left) versus the flux-limiter case using ``downwind'' (\textbf{\texttt{dn}}) and ``far'' points (right)
\end{itemize}

\bigskip
\quad \mbox{\includegraphics[width=0.38\textwidth]{figs/bueler11p8upwind} \hspace{15mm} \includegraphics[width=0.42\textwidth]{figs/bueler11p8fluxlimited}}
\end{frame}
...
\item results on a $40\times 40$ grid with the Koren flux limiter and a Runge-Kutta ODE solver similar to \texttt{ode23}:
...
\item see \texttt{c/ch11/advect.c} at \href{https://github.com/bueler/p4pdes}{\texttt{github.com/bueler/p4pdes}}
    ...
    \end{itemize}
\item parallel run with 4 processors, Koren limiter, and SSP time-stepping:
\begin{Verbatim}[fontsize=\small]
$ mpiexec -n 4 ./advect -adv_problem rotation -da_refine 3 \
    -ts_max_time 6.283185 -ts_monitor_solution draw \
...
\vspace{10mm}
    \begin{itemize}
    \item[$\circ$] compare \texttt{-adv\_limiter none|centered}
    \item[$\circ$] compare \texttt{-ts\_type rk}
    \item[$\circ$] time (without graphics)
    \item[$\circ$] time with \texttt{-da\_refine 5} and $P=1,4$ proc
    \end{itemize}
\end{itemize}
