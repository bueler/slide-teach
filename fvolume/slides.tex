...
\begin{frame}{the reconstruct-evolve-average view}

\begin{itemize}
\item to learn how to kill oscillation, another view is helpful
\item problem is $q_t + a q_x=0$ for $a>0$, where $q(t_n,x)$ has two levels
\item step from $t_n$ (left) to $t_{n+1}$ (right) with first-order upwinding:
...
\end{center}
    ...
    \item[$\circ$] upwinding translates constant values, then computes new cell averages
    \end{itemize}
\item versus Lax-Wendroff, which is equivalent to using the downwind slope:
...
\end{center}
    ...
    \begin{itemize}
    \item[$\circ$] note the \alert{overshoot}
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item so now we focus on ``slope reconstruction'' within cells
