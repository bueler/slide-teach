...
\item \dots but sometimes not so useful even then
    ...
    \item[$\circ$] and some square matrices are defective anyway \dots
    \end{itemize}
\item \emph{any} $A \in \CC^{m\times n}$ has \emph{singular values}
...
\medskip
    Behavior of powers $A^k$ or functions $f(A)$.
...
\begin{itemize}
\item $A$ sends unit ball to hyperellipsoid
\end{itemize}
\end{frame}
