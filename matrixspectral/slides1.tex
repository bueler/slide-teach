...
\vspace{-2mm}
    \begin{itemize}
    \item[$\circ$] the \emph{hermitian conjugate} of $\bv\in\CC^n$ is the row vector $\bv^* = [\overline{v_1},\dots,\overline{v_n}]$
    \end{itemize}
...
with symmetry and positivity properties: \quad for all $\bu,\bv,\bw \in \CC^n$ and $\alpha\in \CC$,
    \begin{itemize}
    \item[$\circ$] $\ip{\bw}{\bu+\bv} = \ip{\bw}{\bu} + \ip{\bw}{\bv}$
    \item[$\circ$] $\ip{\bu}{\alpha\bv} = \alpha \ip{\bu}{\bu}$
    \item[$\circ$] $\ip{\bu}{\bv} = \overline{\ip{\bv}{\bu}}$
    \item[$\circ$] $\ip{\bu}{\bu} \ge 0$ and $\ip{\bu}{\bu}=0$ if and only if $\bu=0$
    \end{itemize}
\end{itemize}
\end{frame}
